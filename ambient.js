...
(function () {
  ...
  // ── Build blobs ──────────────────────────────────────────────────────
  // Palette restricted to brand pink + subtle pastel yellow only.
  const PALETTE = [
    ['#fdf0f4', '#f7d2dc'],   // blush pink (lighter)
    ['#fdf8e4', '#f4ecc8'],   // pastel yellow (lighter)
  ];
  ...
  const blobs = [];
  recipe.forEach(function(b) {
    const colors = pick(PALETTE);
    const rot = rand(-12, 12);
    const el = buildBlob(Object.assign({}, b, { colors: colors, rot: rot }));
  ...
  function tick() {
    ...
    blobs.forEach(function(el) {
      ...
      el.style.webkitTransform = 'translate3d(' + x + 'px, ' + y + 'px, 0) rotate(' + rotNow + 'deg)';
